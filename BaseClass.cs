    ...
    class Competitor
    {
        ...
        public void EffectedByHimeko_Love(Competitor Defender)
        {
            try
            {
                ...
                if(( (ICompetitor) Defender ).GetName() == "KallenAndSakura")
                {
                    ...
                    Console.WriteLine($"{( (ICompetitor) this ).GetName()}发动技能真爱不死，提升100%伤害");
                }
            }
            catch(UserMismatchingException)
            {
        ...
        public void EffectedByHimeko_Alcohol()
        {
            try
            {
                ...
                Console.WriteLine($"{( (ICompetitor) this ).GetName()}发动技能干杯，朋友 提升{Attack - PreviousAttack}点攻击力，降低{PreviousHitRate - HitRate}%命中率");
            }
            catch(UserMismatchingException)
            {
                return;
